...
class PhysicalPipeline:
    ...
    def _check_new_cols(self, cols: list[dict], op_name: str) -> None:
        """Reject a convert (map/sem_map/rag_map) that declares a column name the pipeline already
        has, or the same name twice in one call.

        PZ converts can only ADD columns, never overwrite one. PhysicalOperator computes
        `generated_fields` as (output_schema - input_schema), so a re-declared name is generated by
        nobody, and BOTH outcomes are invisible without this check:

        - Some declared names are new: the op runs, but the re-declared columns silently keep their
          upstream values. The UDF/LLM computes them and PZ discards the answers.
        - EVERY declared name already exists: `generated_fields` is empty, so PZ's
          `_create_data_records_from_field_answers` hits `max([])` and raises "max() iterable
          argument is empty". The pipeline catches that per record, so the op drops EVERY record
          and the plan returns 0 rows.

        The second is what a write-back/coalesce step (`map(coalesce, cols=[...names an earlier
        rag_map already produced...])`) does; the first is worse, because nothing at all is
        reported. Both are caught here, at plan-construction time, where the agent can just rename
        the columns and rewrite the plan. (Same trap as `add_image_data` reusing the id column.)"""
        seen, clashes, repeats = set(), [], []
        for col in cols:
            name = col["name"]
            if name in self._defs:
                ...
                repeats.append(name)
            seen.add(name)
        if not clashes and not repeats:
            return
        problem = []
        if clashes:
            problem.append(f"column(s) {clashes} already exist in this pipeline")
        if repeats:
            problem.append(f"column(s) {repeats} are declared twice in this same call")
        raise ValueError(
            f"{op_name}: {'; '.join(problem)}. An operator can only ADD columns — it never "
            "overwrites one, because PZ generates only the fields that are not already in its "
            "input schema. A repeated name is therefore either silently ignored (its computed "
            "value is discarded and the old value kept) or, if every declared name already "
            "exists, fatal: the operator drops every record and the plan returns 0 rows. "
            "Rewrite the plan so no column name is ever declared twice. To combine several "
            "sources into one final column, give each source its own name (e.g. 'X_rag', "
            "'X_hdr') and produce 'X' itself — a name nothing has used yet — in one later map "
            "that reads them."
        )
    ...
    def sem_map(self, cols: list[dict], model: Model, depends_on: list[str] | None = None, reasoning_effort_override: str | None = None) -> "PhysicalPipeline":
        ...
        cols: list of {"name": str, "type": type, "description": str}
            ...
            Every name must be NEW — see _check_new_cols.
        """
        self._check_new_cols(cols, "sem_map")
        new_defs = {
            col["name"]: (
    ...
    def rag_map(
        ...
        Every name in `cols` must be NEW — see _check_new_cols.
        """
        self._check_new_cols(cols, "rag_map")
        new_defs = {
            col["name"]: (
                Optional[col.get("type", Any)],
                FieldInfo(default=None, description=col["description"]),
